 ...
 */
  ...
  /**
   * Post-login redirect toggle (Login behavior section).
   *
   * Enables the Save button only when the switch differs from the saved
   * value, then POSTs to /dc-config/set-login-redirect on Save.
   */
  Drupal.behaviors.decoupledConfigLoginRedirect = {
    attach: function (context) {
      once('dc-login-redirect', '#dc-config-login-redirect', context).forEach(function (cb) {
        var btn = document.getElementById('dc-config-login-redirect-save');
        var statusEl = document.getElementById('dc-config-login-redirect-status');
        if (!btn) { return; }
        var saved = cb.checked;

        function setStatus(text, cls) {
          if (!statusEl) { return; }
          statusEl.textContent = text || '';
          statusEl.className = 'dc-config-setting-status' + (cls ? ' ' + cls : '');
        }

        cb.addEventListener('change', function () {
          btn.disabled = (cb.checked === saved);
          if (!btn.disabled) { setStatus('', ''); }
        });

        btn.addEventListener('click', function () {
          var cfg = (typeof drupalSettings !== 'undefined' && drupalSettings.dcConfig) || {};
          var body = new URLSearchParams();
          body.set('enabled', cb.checked ? '1' : '0');
          body.set('form_token', cfg.loginRedirectToken || '');
          btn.disabled = true;
          setStatus('Saving…', '');
          fetch('/dc-config/set-login-redirect', {
            method: 'POST',
            ...
            .then(function (d) {
              if (d && d.success) {
                ...
                setStatus('Error: ' + ((d && d.error) || 'failed'), 'is-error');
                btn.disabled = false;
              }
            })
            .catch(function () {
              setStatus('Error saving.', 'is-error');
              btn.disabled = false;
            });
        });
      });
